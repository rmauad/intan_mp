library(dplyr)
library(ggplot2)

plot_data <- data_intan %>%
  filter(year >= 1975) %>%
  group_by(year, ff_5ind) %>%
  mutate(ppent_real = (ppent/cpi)*100,
         agg_org_cap = sum(org_cap_comp, na.rm = TRUE),
         agg_ppent = sum(ppent_real, na.rm = TRUE),
         o_k = agg_org_cap/agg_ppent)

         #o_k_1975 = ifelse(year == 1975, o_k, NA),
         #o_k_1975 = coalesce(o_k_1975, first(o_k_1975)))

o_k_1975 <- plot_data %>%
  ungroup() %>%
  filter(year == 1975) %>%
  mutate(o_k_1975 = o_k) %>%
  select(o_k_1975, ff_5ind)

o_k_1975 <- unique(o_k_1975)

plot_data <- inner_join(plot_data, o_k_1975, by = "ff_5ind")
  
plot_data_cons <- plot_data %>%
  filter(ff_5ind == 1) %>%
  mutate(o_k_1975_norm = o_k/o_k_1975)

# CONSUMER DOES NOT MATCH
ggplot(data = plot_data_cons, aes(x = year, y = o_k_1975_norm)) +
  geom_line()


  


