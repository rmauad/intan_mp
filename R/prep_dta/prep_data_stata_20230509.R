...
#Treating the data
data_intan <- data_intan %>% select(GVKEY, cusip, year, sic, at, ceq, dlc, 
                                    dltt, ppegt, che, org_cap_comp, csho, 
                                    sale, ib, dp, ppent, CPI, GDP, Ind_prod, 
                                    prcc_c, txdb, dltis) %>%
...
db_reg <- db_reg %>% group_by(GVKEY, year) %>% filter(row_number()==1) #keep just the first observation
