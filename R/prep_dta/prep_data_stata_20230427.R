...
#Treating the data
data_intan <- data_intan %>% select(GVKEY, cusip, year, sic, xsga, sga, intan, at, ceq, dlc, dltt, ppegt, revt, che, org_cap_comp, lt, csho, prcc_f, sale, ib, dp, ppent, capx, pstkl, invt, emp, xint, CPI, GDP, Ind_prod, fatb, prcc_c, txdb, ppent, ib, dp, ppent, dltis) %>%
 ...
 # mutate(dep_hv = dpacb/fatb) %>% #accumulated depreciation/historical value of buildings
  ungroup() %>%
  group_by(year) %>%
  mutate(bankruptcy = ifelse(year == 2020,0,bankruptcy))

# nyears <- data_intan %>% group_by(GVKEY) %>%summarise(nyears = n())
# 
# data_intan <- inner_join(data_intan, nyears, by = 'GVKEY')
# 
# data_intan <- data_intan %>% filter(nyears >= 6)
...
#save(db_reg, file = "data/rdata/db_reg.RData")
#write.dta(db_reg_1989, "data/dta/db_reg_1989_only.dta") #only firms in 1989
write.dta(db_reg, "data/dta/db_reg.dta") #all firms
