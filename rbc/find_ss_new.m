...
function eqs = find_ss_new(x, beta, delta, B, theta, g_ss, phi)
...
eqs = [
    (1 - phi + rf_ss*phi) + beta*(1-theta)*(theta/r_ss)^(theta/(1-theta))/(C_ss*g_ss*B);
    (rn_ss - phi*rf_ss)*(NP_ss) - phi*rf_ss*(1-(1/g_ss))*(MP_ss);
    -(C_ss*g_ss*B/beta)*H_ss*phi - NP_ss - (1-1/g_ss)*MP_ss;
