...
// gen JKff3_q_lag1 = l1.JKff3_q //Paper by John Rogers
// gen JKff3_q_lag2 = l2.JKff3_q
...
* Nakamura-Steinsson (INCLUDE WITH ONE LAG) - Interaction term is negative.
...
xtreg dln_emp c.ns_lag1##c.med c.cf b_pred_med sales_growth_lag1 cash_at CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) //capx_ppe does not work. With one lag, the interaction term is still positive, but I lose significance of the term on its own.
est store Median
xtreg dln_emp c.ns_lag1##c.tercile c.cf b_pred_ter sales_growth_lag1 cash_at CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Tercile
xtreg dln_emp c.ns_lag1##c.quartile c.cf b_pred_qua sales_growth_lag1 cash_at CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Quartile
xtreg dln_emp c.ns_lag1##c.quintile c.cf b_pred_qui sales_growth_lag1 cash_at CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Quintile
xtreg dln_emp c.ns_lag1##c.decile c.cf b_pred_dec sales_growth_lag1 cash_at CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust)
est store Decile
//outreg2 [Median Tercile Quartile Quintile Decile] using output/tex/investment.tex, replace label drop(i.year) dec(3) nocons alpha(0.01, 0.05, 0.1, 0.15) symbol(***, **, *, +)
...
est store Decile
//outreg2 [Median Tercile Quartile Quintile Decile] using output/tex/investment.tex, replace label drop(i.year) dec(3) nocons alpha(0.01, 0.05, 0.1, 0.15) symbol(***, **, *, +)
esttab Median Tercile Quartile Quintile Decile using output/tex/emp_brw.tex, b(%9.3f) label replace se ar2 stats(N r2, fmt(0 %9.3f)) star(+ 0.10 * 0.05 ** 0.01 *** 0.001) drop(*.year med tercile quartile quintile decile b_pred_med b_pred_ter b_pred_qua b_pred_qui b_pred_dec brw_lag2 c.brw_lag2#c.med c.brw_lag2#c.tercile c.brw_lag2#c.quartile c.brw_lag2#c.quintile c.brw_lag2#c.decile)
...
xtreg dln_emp c.brw_lag1##c.med  c.cf sales_growth_lag1 cash_at CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Median
xtreg dln_emp c.brw_lag1##c.tercile  c.cf sales_growth_lag1 cash_at CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Tercile
xtreg dln_emp c.brw_lag1##c.quartile  c.cf sales_growth_lag1 cash_at CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Quartile
xtreg dln_emp c.brw_lag1##c.quintile  c.cf sales_growth_lag1 cash_at CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Quintile
xtreg dln_emp c.brw_lag1##c.decile  c.cf sales_growth_lag1 cash_at CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust)
est store Decile
//outreg2 [Median Tercile Quartile Quintile Decile] using output/tex/investment.tex, replace label drop(i.year) dec(3) nocons alpha(0.01, 0.05, 0.1, 0.15) symbol(***, **, *, +)
...
xtreg dln_emp c.JKff3_q_lag1##c.med c.JKff3_q_lag2##c.med b_pred_med c.cf sales_growth_lag1 CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Median
xtreg dln_emp c.JKff3_q_lag1##c.tercile c.JKff3_q_lag2##c.tercile b_pred_ter c.cf sales_growth_lag1 CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Tercile
xtreg dln_emp c.JKff3_q_lag1##c.quartile c.JKff3_q_lag2##c.quartile b_pred_qua c.cf sales_growth_lag1 CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Quartile
xtreg dln_emp c.JKff3_q_lag1##c.quintile c.JKff3_q_lag2##c.quintile b_pred_qui c.cf sales_growth_lag1 CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Quintile
xtreg dln_emp c.JKff3_q_lag1##c.decile c.JKff3_q_lag2##c.decile b_pred_dec c.cf sales_growth_lag1 CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust)
est store Decile
//outreg2 [Median Tercile Quartile Quintile Decile] using output/tex/investment.tex, replace label drop(i.year) dec(3) nocons alpha(0.01, 0.05, 0.1, 0.15) symbol(***, **, *, +)
esttab Median Tercile Quartile Quintile Decile using output/tex/emp_JKff3.tex, b(%9.3f) label replace se ar2 stats(N r2, fmt(0 %9.3f)) star(+ 0.10 * 0.05 ** 0.01 *** 0.001) drop(*.year med tercile quartile quintile decile b_pred_med b_pred_ter b_pred_qua b_pred_qui b_pred_dec JKff3_q_lag1 c.JKff3_q_lag1#c.med c.JKff3_q_lag1#c.tercile c.JKff3_q_lag1#c.quartile c.JKff3_q_lag1#c.quintile c.JKff3_q_lag1#c.decile)
