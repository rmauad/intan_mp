...
est store Decile
esttab Median Tercile Quartile Quintile Decile using /homes/nber/mauadr/bulk/orbis.work/intan_mp/output/emp_JKff3.tex, b(%9.3f) label replace se ar2 stats(N r2, fmt(0 %9.3f)) star(+ 0.10 * 0.05 ** 0.01 *** 0.001) drop(*.year med tercile quartile quintile decile JKff3_q_lag1 c.JKff3_q_lag1#c.med c.JKff3_q_lag1#c.tercile c.JKff3_q_lag1#c.quartile c.JKff3_q_lag1#c.quintile c.JKff3_q_lag1#c.decile)
...
est store Decile
esttab Median Tercile Quartile Quintile Decile using output/tex/investment.tex, b(%9.3f) label replace se ar2 stats(N r2, fmt(0 %9.3f)) star(+ 0.10 * 0.05 ** 0.01 *** 0.001) drop(*.year med tercile quartile quintile decile MP_S_LSAPfactor_lag2 c.MP_S_LSAPfactor_lag2##c.med c.MP_S_LSAPfactor_lag2##c.tercile c.MP_S_LSAPfactor_lag2##c.quartile c.MP_S_LSAPfactor_lag2##c.quintile c.MP_S_LSAPfactor_lag2##c.decile)
