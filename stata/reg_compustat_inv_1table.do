...
est store brw
xtreg inv_tot_at c.ns_lag1##c.tercile b_pred_ter c.cf sales_growth_lag1 cash_at CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) //capx_ppe does not work. With one lag, the interaction term is still positive, but I lose significance of the term on its own.
est store ns
...
xtreg inv_tot_at c.mpshock##c.tercile c.brw_lag2##c.tercile b_pred_ter c.cf sales_growth_lag1 cash_at CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store brw
cap rename (mpshock) (brw_lag1)
cap rename (ns_lag1) (mpshock)
xtreg inv_tot_at c.mpshock##c.tercile b_pred_ter c.cf sales_growth_lag1 cash_at CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store ns
...
xtreg inv_tot_at c.mpshock##c.tercile c.MP_S_LSAPfactor_lag2##c.tercile b_pred_ter c.cf sales_growth_lag1 cash_at CPI RGDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store LSAPfactor
cap rename (mpshock) (MP_S_LSAPfactor_lag1)

//esttab brw ns jk Fffactor Fgfactor LSAPfactor using output/tex/inv_all_shocks_tercile.tex, b(%9.3f) label replace se ar2 stats(N r2, fmt(0 %9.3f)) star(* 0.10 ** 0.05 *** 0.01) drop(*.year b_pred_ter tercile brw_lag1 brw_lag2 c.brw_lag2#c.tercile ns_lag1 JKff3_q_lag1 MP_S_Fffactor_lag1 MP_S_Fgfactor_lag1 MP_S_LSAPfactor_lag1 MP_S_LSAPfactor_lag2 c.MP_S_LSAPfactor_lag2#c.tercile cf sales_growth_lag1 cash_at CPI RGDP Ind_prod)
esttab brw ns jk Fffactor Fgfactor LSAPfactor using output/tex/inv_all_shocks_tercile.tex, coeflabels(mpshock "MPshock(t-1)" c.mpshock#c.tercile "MPshock(t-1) x Tercile") b(%9.3f) label replace se ar2 stats(N r2, fmt(0 %9.3f)) star(* 0.10 ** 0.05 *** 0.01) drop(*.year b_pred_ter tercile brw_lag2 c.brw_lag2#c.tercile MP_S_LSAPfactor_lag2 c.MP_S_LSAPfactor_lag2#c.tercile cf sales_growth_lag1 cash_at CPI RGDP Ind_prod)
