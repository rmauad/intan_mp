...
merge 1:m bvdidnumber using /homes/nber/mauadr/bulk/orbis.work/intan_mp/data/dta/sales/intan_inv_ctrl.dta, keep(3) nogen
...
// capitalexpenditures and purchaseacquisitionofintangibles are negative in Orbis (representing a capital expending).
...
egen med = xtile(intan_sales), by(year ff_indust) n(2)
egen tercile = xtile(intan_sales), by(year ff_indust) n(3)
egen quartile = xtile(intan_sales), by(year ff_indust) n(4)
egen quintile = xtile(intan_sales), by(year ff_indust) n(5)
egen decile = xtile(intan_sales), by(year ff_indust) n(10)

save /homes/nber/mauadr/bulk/orbis.work/intan_mp/data/dta/sales/intan_sales_comp.dta, replace
// Merging databases
merge m:1 year using /homes/nber/mauadr/bulk/orbis.work/intan_mp/data/dta/sales/db_reg_y_orbis.dta, keep(3) nogen //only keep matched observations
//drop if year == year[_n-1] //in case there are repeated time values within the panel
save /homes/nber/mauadr/bulk/orbis.work/intan_mp/data/dta/sales/db_reg_sales_orbis.dta, replace

// Selecting non-publicly traded firms
use bvdidnumber stockexchangeslisted using exchanges.dta
duplicates drop bvdidnumber, force
merge 1:m bvdidnumber using /homes/nber/mauadr/bulk/orbis.work/intan_mp/data/dta/sales/db_reg_sales_orbis.dta, keep(2) //keep (2) preserves the observations from b_reg_orbis.dta that did not have an equivalent in exchanges.dta
save /homes/nber/mauadr/bulk/orbis.work/intan_mp/data/dta/sales/db_reg_sales_orbis_npt.dta, replace
