...
xtreg log_ltdebt_issue_at c.ir_diff_delta##c.med c.mb c.cf sales_growth_lag1 cash_at CPI GDP Ind_prod i.year if year >= 1990, fe cluster(ff_indust) // the interaction term is positive, but the term on its own is insignificant. Insignificant with ir_diff_10y_2y or ir_diff_10y_2y_lag1
est store Median_lev
xtreg log_ltdebt_issue_at c.ir_diff_delta##c.tercile c.mb c.cf sales_growth_lag1 cash_at CPI GDP Ind_prod i.year if year >= 1990, fe cluster(ff_indust) 
est store Tercile_lev
xtreg log_ltdebt_issue_at c.ir_diff_delta##c.quartile c.mb c.cf sales_growth_lag1 cash_at CPI GDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Quartile_lev
xtreg log_ltdebt_issue_at c.ir_diff_delta##c.quintile c.mb c.cf sales_growth_lag1 cash_at CPI GDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Quintile_lev
xtreg log_ltdebt_issue_at c.ir_diff_delta##c.decile c.mb c.cf sales_growth_lag1 cash_at CPI GDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust)
...
xtreg inv_tot_at c.ffrate_zlb_lag1##c.med c.ffrate_zlb_lag2##c.med c.mb c.cf sales_growth_lag1 cash_at CPI GDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) //capx_ppe does not work. With one lag, the interaction term is still positive, but I lose significance of the term on its own.
est store Median
xtreg inv_tot_at c.ffrate_zlb_lag1##c.tercile c.ffrate_zlb_lag2##c.tercile c.mb c.cf sales_growth_lag1 cash_at CPI GDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Tercile
xtreg inv_tot_at c.ffrate_zlb_lag1##c.quartile c.ffrate_zlb_lag2##c.quartile c.mb c.cf sales_growth_lag1 cash_at CPI GDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Quartile
xtreg inv_tot_at c.ffrate_zlb_lag1##c.quintile c.ffrate_zlb_lag2##c.quintile c.mb c.cf sales_growth_lag1 cash_at CPI GDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust) 
est store Quintile
xtreg inv_tot_at c.ffrate_zlb_lag1##c.decile c.ffrate_zlb_lag2##c.decile c.mb c.cf sales_growth_lag1 cash_at CPI GDP Ind_prod i.year if year >= 1990,fe cluster(ff_indust)
est store Decile
